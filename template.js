...
function getPokemonOverlayHTML(pokemon) {
    const typeClass = getTypeClass(pokemon);
...
function getTypeClass(pokemon) {
    ...
    return pokemon.types.map(t => typeColors[t.type.name] || '').join(' ');
}

function getStatsHTML(pokemon) {
    return ['hp', 'attack', 'defense', 'speed'].map(statName => statBar(statName.toUpperCase(), getStatValue(pokemon, statName))).join('');
}
...
function statBar(label, value, max = 101) { 
    const percent = Math.min(101, Math.round((value / max) * 100));
    return `
        <div class="stat-bar-row">
            <span class="stat-label">${label}:</span>
            <div class="stat-bar-bg">
                <div class="stat-bar-fill" style="width:${percent}%; background-color: ${getBarColor(percent)};" title="${value}"></div>
            </div>
            <span class="stat-value">${value}%</span> <!-- Prozentwert anzeigen -->
        </div>
    `;
}

function getBarColor(percent) {
    if (percent > 75) return '#151313ff'; 
    if (percent > 50) return '#1f1d1dff'; 
    return '#131010ff'; 
}
